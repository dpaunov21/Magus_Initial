...
public class CameraController : MonoBehaviour
{
    ...
    // Camera Variables
    public float cameraHeight = 1.75f, cameraMaxDistance = 25f;
    ...
    void Update()
    {
        if (!Input.GetKey(leftMouse) && !Input.GetKey(rightMouse) && !Input.GetKey(middleMouse))  // if no mouse button pressed
            cameraState = CameraState.CameraNone;
        else if (Input.GetKey(leftMouse) && !Input.GetKey(rightMouse) && !Input.GetKey(middleMouse))  // if left mouse button pressed
            cameraState = CameraState.CameraRotate;
        else if (!Input.GetKey(leftMouse) && Input.GetKey(rightMouse) && !Input.GetKey(middleMouse))  // if right mouse button pressed
            cameraState = CameraState.CameraSteer;
        else if ((Input.GetKey(leftMouse) && Input.GetKey(rightMouse)) || Input.GetKey(middleMouse))  // if left and right mouse or middle mouse is pressed
    ...
    void CameraInputs()
    {
        if(cameraState != CameraState.CameraNone)
        {
            ...
            {
                if (!camYAdjust && camMoveState != CameraMoveState.NeverAdjust)
                    camXAdjust = true;
        ...
        if(cameraState != CameraState.CameraRotate)
        {
            if(camXAdjust)
            {
                ...
                if (Mathf.Abs(panAngle) > rotationXCushion)
                    currentPan = Mathf.Lerp(currentPan, currentPan + panAngle, 0.1f);
